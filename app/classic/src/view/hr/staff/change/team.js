Ext.define('APP.view.hr.staff.change.team',{
    extend:'Ext.Panel',
	...
	items:[
		{xtype:'grid',region:'west',title:'在职员工',collapsible:true,width:500,
			...
			columns:[
				...
				{text:'现属团队',xtype:'templatecolumn',dataIndex:'teamid',width:160,tpl:'{team_name}<r>{manager_namecn}</r>'}
			],
			viewConfig:{enableTextSelection:false},
			features:[{ftype:'grouping'},{ftype:'summary',dock:'bottom'}],
			listeners:{
				itemdblclick:'onChangeTeamClick'
			}
	   	},
		{xtype:'grid',region:'center',title:'人事调岗记录',
			reference:'gridChange',
			  store:{
				type:'hrStaffChangeTeam',
				autoLoad:true,
			...
			columns:[
				...
				{text:'姓名',xtype:'templatecolumn',dataIndex:'namecn',minWidth:180,flex:1,tpl:'{namecn} <s>{name}</s> <r>{post_name}</r>',summaryType:'count'},
				{text:'工号',dataIndex:'login',width:100,renderer:'returnLogin'},
				{text:'调动前',xtype:'templatecolumn',dataIndex:'before',width:200,flex:1,tpl:'{before_department_name} · {before_team_name}'},
				{text:'调动后',xtype:'templatecolumn',dataIndex:'after',width:200,flex:1,tpl:'{after_department_name} · {after_team_name}'},
				{text:'生效日期',dataIndex:'effectivedate',width:100},
